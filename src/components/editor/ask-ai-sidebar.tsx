...
export function AskAISidebar({ isOpen, onClose, editor }: AskAISidebarProps) {
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      // Pass the entire document context as requested for V1
      const documentContext = editor.getText();
      
      const result = await askAi({ data: { prompt: text, documentContext } });

      if (!controller.signal.aborted) {
        const aiMsg: Message = { id: crypto.randomUUID(), role: "ai", content: result.response };
...
function MessageBubble({ message, editor, isLast, onRegenerate }: { message: Message; editor: Editor | null; isLast: boolean; onRegenerate: () => void }) {
